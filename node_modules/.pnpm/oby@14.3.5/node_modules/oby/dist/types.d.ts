...
type TryCatchFunction<T = unknown> = ({ error, reset }: {
    error: Error;
    reset: DisposeFunction;
}) => T;
...
type Callable<T extends CallableFunction> = (T & {
    call: CallableFunctionCall<T>;
}) | ({
    call: CallableFunctionCall<T>;
    apply?: never;
});
...
type Constructor<T = unknown, Arguments extends unknown[] = []> = {
...
type ResolvableObject = {
    [Key in string | number | symbol]?: Resolvable;
};
